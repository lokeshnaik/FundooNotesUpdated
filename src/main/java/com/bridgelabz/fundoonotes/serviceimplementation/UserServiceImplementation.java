...
public class UserServiceImplementation implements UserServices 
{
	...
	public User userRegistration(UserInformationdto informationdto) throws UserException 
	{
		...
		if (userRepository.getUser(informationdto.getEmail()).isPresent() == false) {
		   ...
		   user.setPassword(encrptPassword);
			...
			String mailResponse = response.fromMessage("http://localhost:8080/verify",jwtop.JwtToken(user.getUserId()));
	...
	public boolean forgotPassword(String email) throws UserException 
	{
		...
		User user = repository.getUser(email).orElseThrow(() -> new UserException("User not registered", HttpStatus.NOT_FOUND));
